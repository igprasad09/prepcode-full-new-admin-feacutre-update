...
const LikeButton = ({totalLikes} : {totalLikes: number | string}) => {
   
  const profileEmail = useRecoilValue(profileEmailAtom);
  const [liked, setLiked] = useState(false);
  const {id} = useParams(); 
  
  function AddLike(e: React.MouseEvent) {
          e.preventDefault();
          e.stopPropagation();
        ...
        async function AddLikeInBackend() {
            await axios.post("http://localhost:3000/programs/addlikes",{
                 email: profileEmail,
                 id
            }).then((res)=>{
        ...
        async function RemoveLikeInBackend() {
            console.log("dislike")
        }

  return (
    <StyledWrapper>
      <div className={`like-dislike-container ${profileEmail ? 'logged-in' : ''}`}>			
        <div className="icons-box">
          <div className="icons">
            <label className="btn-label" htmlFor="like-checkbox" onClick={AddLike}>
              <span className="like-text-content">{totalLikes}</span>
              <input className="input-box" id="like-checkbox" type="checkbox" checked={liked} readOnly/>
...
const StyledWrapper = styled.div`
  ...
  /* Fireworks only fires when logged in */
  .like-dislike-container.logged-in .icons #like-checkbox:checked ~ .fireworks > .checked-like-fx {
    position: absolute;
    width: 10px;
    height: 10px;
    right: 40px;
